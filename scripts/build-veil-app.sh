...
install -m 0755 "$root/target/release/cocoa-way" "$app/Contents/MacOS/Veil"
install -m 0644 "$root/packaging/Veil-Info.plist" "$app/Contents/Info.plist"
iconwork=$(mktemp -d "${TMPDIR:-/tmp}/veil-display.XXXXXX")
iconset="$iconwork/Veil.iconset"
mkdir -p "$iconset"
